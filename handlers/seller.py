...
async def cmd_my_deals(message: Message) -> None:
    ...
    from models.database import get_deals_for_seller
    
    try:
        deals = await get_deals_for_seller(user["id"])
        ...
        for deal in deals[:10]:
            text += f"♻️ {deal.get('fkko_name', 'Неизвестно')[:30]}\n"
            text += f"   Заявка #{deal['id']} | 💰 {deal.get('transport_cost', 0):,.0f} ₽\n\n"
